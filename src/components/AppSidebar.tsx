...
import { loadProfile, loadStreak, type ProfileData, type StreakData } from "@/lib/progress-store";
  ...
  return (
    <>
      <aside className="hidden lg:flex w-64 shrink-0 flex-col border-r border-sidebar-border bg-sidebar/80 backdrop-blur-xl">
        ...
        <div className="mx-3 mb-2 rounded-xl glass p-4 space-y-2">
        ...
        <div className="mx-3 mb-3 rounded-xl glass p-4 space-y-2">
          <div className="flex items-center justify-between text-xs text-muted-foreground">
            <span>⚡ Level {profile.level}</span>
            <span className="tabular-nums">{profile.xp} / {profile.xpToNext} XP</span>
          </div>
          <div className="h-1.5 w-full rounded-full bg-white/5 overflow-hidden">
            <div className="h-full gradient-primary rounded-full transition-all duration-500"
              style={{ width: `${xpPct}%` }} />
          </div>
          <p className="text-[11px] text-muted-foreground">+50 XP per chapter · +500 XP per subject</p>
        </div>
