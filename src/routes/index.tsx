      ...
      <div className="p-4 sm:p-6 lg:p-8 space-y-4 max-w-350 mx-auto w-full">
        <div className="grid grid-cols-4 gap-3 sm:gap-4 stagger-children">
          ...
          <article className="col-span-1 rounded-2xl p-3 sm:p-4 glass flex flex-col justify-between aspect-square sm:aspect-auto sm:min-h-27.5 hover-lift">
            ...
            <div>
              <div className="font-display text-2xl sm:text-3xl font-bold text-white leading-none">{streak.current}</div>
          ...
          <article className="col-span-3 rounded-2xl p-4 sm:p-5 glass flex items-center justify-between hover-lift">
            {nextExam ? (
              ...
              <div className="text-sm text-muted-foreground">No exams scheduled yet.</div>
            )}
          </article>
            ...
            <div className="flex justify-between items-center mb-4">
              <div>
                ...
                <p className="text-[11px] text-zinc-500 mt-0.5">
                  {todayPlan.length > 0
                    ? `${todayPlan.length} sessions · ${todayPlan.reduce((s, x) => s + x.durationHours, 0).toFixed(1)}h total`
                ...
                {todayPlan.slice(0, 4).map((s, i) => (
                  <div key={i} className={`flex items-center gap-3 p-2.5 rounded-xl transition ${s.done ? "opacity-50" : "hover:bg-white/3"}`}>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-[10px] text-zinc-500 truncate">{s.subjectName}</div>
                    </div>
          ...
          <article className="col-span-4 rounded-[1.75rem] p-5 glass">
            <div className="flex justify-between items-center mb-5">
              ...
              <span className="text-[10px] font-medium px-2 py-0.5 bg-white/10 text-zinc-300 rounded-full">Live Data</span>
            </div>
            {noSubjects ? (
              ...
              <div className="space-y-4">
                {subjects.slice(0, 4).map((s, i) => {
                  const tones = [
                    { bg: "bg-white/10", br: "border-white/20", tx: "text-zinc-300", bar: "bg-white" },
                    { bg: "bg-zinc-500/10", br: "border-zinc-500/20", tx: "text-zinc-400", bar: "bg-zinc-400" },
                    { bg: "bg-zinc-600/10", br: "border-zinc-600/20", tx: "text-zinc-500", bar: "bg-zinc-500" },
                    { bg: "bg-zinc-700/10", br: "border-zinc-700/20", tx: "text-zinc-600", bar: "bg-zinc-600" },
                  ][i] ?? { bg: "bg-zinc-700/10", br: "border-zinc-700/20", tx: "text-zinc-600", bar: "bg-zinc-600" };
                  return (
          ...
          <article className="col-span-2 rounded-2xl p-4 sm:p-5 glass hover-lift">
            ...
            <div className="flex items-end gap-1 h-14">
              {weeklyHours.map((d, i) => {
                ...
                return (
                  <div key={i}
                    className={`flex-1 rounded-t-sm transition-all duration-700 ${isPeak ? "bg-white" : "bg-[#1a1a1a]"}`}
                    style={{ height: `${h}%`, boxShadow: isPeak ? "0 0 10px rgba(255,255,255,0.3)" : undefined }}
                  />
                );
              })}
            </div>
            <div className="flex items-end justify-between mt-3">
              <div>
                <div className="text-base font-bold text-white font-display leading-none tabular-nums">{totalHours.toFixed(1)}</div>
            ...
            <div className="relative flex items-center gap-3">
              <div className="relative h-14 w-14 shrink-0">
                <svg viewBox="0 0 60 60" className="-rotate-90 h-full w-full">
                  ...
                  <circle cx="30" cy="30" r="25" strokeWidth="5" stroke="url(#gr)" strokeLinecap="round" fill="none"
                    strokeDasharray={`${(stats.avgCompletion / 100) * 157} 157`} />
                  <defs>
                ...
                <div className="absolute inset-0 grid place-items-center">
                  <span className="text-xs font-bold font-display text-white">{stats.avgCompletion}%</span>
                </div>
              </div>
              <div className="min-w-0">
                <span className="text-[10px] uppercase font-bold tracking-wider text-zinc-500">Avg Progress</span>
                <div className="font-display text-base font-bold text-white mt-0.5">
                  {stats.avgCompletion >= 75 ? "Strong" : stats.avgCompletion >= 50 ? "Good" : "Keep going"}
                </div>
                <div className="text-[10px] text-zinc-300 font-mono mt-0.5">{stats.totalChaptersDone}/{stats.totalChapters} chapters</div>
              </div>
            </div>
          </article>
          ...
          <article className="col-span-2 rounded-2xl p-4 sm:p-5 glass hover-lift">
            ...
            <div className="flex items-baseline gap-1">
              <span className="font-display text-3xl font-bold text-white tabular-nums leading-none">
                {todayPlan.filter(s => s.done).reduce((sum, s) => sum + s.durationHours, 0).toFixed(1)}
              </span>
            ...
            <div className="text-[10px] text-slate-500 mt-2">
              {todayPlan.filter(s => !s.done).length} sessions remaining
            </div>
          </article>
        </div>
          ...
          <article className="col-span-4 sm:col-span-2 rounded-[1.75rem] p-5 glass">
            <div className="flex items-center gap-2 mb-4">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-500">Based on your study data</p>
              </div>
            </div>
            <div className="space-y-2.5">
